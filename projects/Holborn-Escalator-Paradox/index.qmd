...
The simulation runs for 13,560 ticks — 120 minutes at 113 ticks/min — comprising a 30-minute warm-up, a 60-minute rush hour (08:30–09:30), and a 30-minute cool-down. Five submodels execute sequentially each tick:
...
The policy implication follows directly from that last result. TfL's standing-only trial succeeds not because it eliminates walking, but because it eliminates the norm that prevents standers from using both lanes. 

This reframing opens up a wider set of interventions than a blanket ban. If the binding constraint is norm adherence rather than walking speed, then measures that interrupt the keep-right habit specifically during peak congestion such as signage, floor markings, platform announcements and staff direction,  might capture a substantial share of the same benefit without removing walking as an option during quieter periods. The model suggests the gains are available wherever lane-switching becomes responsive to congestion.
