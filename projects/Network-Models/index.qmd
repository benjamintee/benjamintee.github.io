...
ax1.set_title('Largest Connected Component Size', fontsize=14, fontweight='bold', pad=10)
...
ax2.set_title('Number of Connected Components', fontsize=14, fontweight='bold', pad=10)
...
for label, df in measures.items():
    ax3.plot(df['Iteration'], df['Diameter of LCC'],
             marker='o', markersize=5, linewidth=2,
             color=colours[label], label=label)
ax3.set_title('Diameter of Largest Component', fontsize=14, fontweight='bold', pad=10)
...
#| warning: false
#| label: tbl-best-walk
#| tbl-cap: "Alternative stations ranked by shortest walking time"
#| output: asis
...
`{python} best_total['Total Time (mins)']` minutes.  

Walking times across all candidates range from
`{python} df_final['Walking Time (mins)'].min()` to
`{python} df_final['Walking Time (mins)'].max()` minutes, so the burden of disruption falls
unevenly depending on where within the station's catchment a passenger begins.
...
Representing stations as nodes and track as edges is a simple but powerful concept. It turns questions that would otherwise rest on operational intuition into ones that can be computed via centrality measures which show which stations carry structural weight. Sequential removal reveals how much the network can lose before it fractures, and shortest-path algorithms find viable alternatives when a station closes. A handful of algorithms answered questions about criticality, vulnerability and routing across a network of 400 stations.

The most useful finding was that the measures disagree. Stratford leads the topological rankings but falls to sixth once passenger flows are attached, while Green Park rises to first. Neither is wrong, they just describe different kinds of importance. A station can be structurally pivotal while carrying modest demand, or the reverse. For planning, that means taking into account both views when deploying resilience enhancement measures. 
